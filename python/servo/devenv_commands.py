...
class MachCommands(CommandBase):
    def run_cargo(self, params, geckolib=False, check=False):
        ...
        if params and params[0] in [
            # In a Cargo source tree:
            # grep -l manifest-path src/bin/* | sed -e 's_^.*/\(.*\)\.rs_"\1",_' -e s/_/-/
            "bench", "build", "check", "clean", "doc", "fetch", "generate-lockfile",
            "locate-project", "metadata", "package", "pkgid", "publish", "read-manifest",
            "run", "rustc", "rustdoc", "test", "update", "verify-project",
        ]:
            params[1:1] = ["--manifest-path",
                           self.geckolib_manifest() if geckolib else self.servo_manifest()]
